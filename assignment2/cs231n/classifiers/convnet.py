...
class DeepNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_conv=2, num_affine=2, num_filters=32, filter_size=7,
               hidden_dim=[100], num_classes=10, weight_scale=1e-3, reg=0.0, dropout=0.7, use_batchnorm=True,
...
#   conv-relu-pool x N && conv-relu parameters
    for i in np.arange(1, num_conv+2):
      ...
      if self.use_batchnorm and i!=num_conv+1:
        self.params['gamma%d' % float(i)] = np.random.randn(num_filters)
        self.params['beta%d' % float(i)] = np.random.randn(num_filters)
      C = num_filters
  ...
  def loss(self, X, y=None):
    ...
    for i in np.arange(1, num_conv+1):
      ...
      out, conv_cache = conv_forward_fast(out, W, b, conv_param)
      temp_cache.append(conv_cache)
      # batchnorm forward
      if self.use_batchnorm:
        gamma, beta = self.params['gamma%d' % float(i)], self.params['beta%d' % float(i)]
        out, bn_cache = spatial_batchnorm_forward(out, gamma, beta, self.bn_params[i-1])
        temp_cache.append(bn_cache)
      # relu forward
      out, relu_cache = relu_forward(out)
      temp_cache.append(relu_cache)
      # dropout
      if self.use_dropout:
        out, dropout_cache = dropout_forward(out, self.dropout_param)
        temp_cache.append(dropout_cache)
      # pool
      out, pool_cache = max_pool_forward_fast(out, pool_param)
      temp_cache.append(pool_cache)
    ...
    # conv-bn-relu-dropout-pool backwards
    for i in np.arange(num_conv-1, -1, -1):
      ...
      if (not self.use_batchnorm and not self.use_dropout):
        ...
        iter = -1
        # pool
        dout = max_pool_backward_fast(dout, cache_list[i][iter])
        iter -= 1
        # dropout
        if self.use_dropout:
          dout = dropout_backward(dout, cache_list[i][iter])
          iter -= 1
        # relu
        dout = relu_backward(dout, cache_list[i][iter])
        iter -= 1
        if self.use_batchnorm:
            dout, dgamma, dbeta = spatial_batchnorm_backward(dout, cache_list[i][iter])
            grads['gamma%d' % float(i+1)] = dgamma
            grads['beta%d' % float(i+1)] = dbeta
            iter -= 1
        dout, dw, db = conv_backward_fast(dout, cache_list[i][iter])
        grads['W%d' % float(i+1)] = dw + self.reg * self.params['W%d' % float(i+1)]
        grads['b%d' % float(i+1)] = db
